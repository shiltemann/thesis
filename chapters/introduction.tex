...
Similar concerns on the more purely informatics side of bioinformatics as well, take the example of bash scripts; many OS-dependent syntax variations can hinder interoperabililty, but by taking care to comply with POSIX standards, such concerns can be mitigated.
...
Mutations in DNA are described relative to a \emph{reference genome}, and comparing variants across different samples is inherently challenging because there are typically a number of equally valid ways to describe a variant. Variants in DNA are described relative to

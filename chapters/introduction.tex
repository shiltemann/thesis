...
And this is not just a problem within a single institution; far too often, results described in journal articles are also impossible to reproduce given the information available in the manuscript.
...
As we have seen, the choice of software can impact downstream analysis, and variant calling is no exception. Different aligners lead employ different strategies, which leads to different variant calls for the same observed sequence depending on the choice of mapping and calling algorithms \cite{zook2014integrating} (Figure \ref{fig:variant-multiple-representations}) This greatly complicates comparisons of variants across studies
...
some can be mitigated via variant normalisation (left-aligning etc), but no consensus (left aligned prevalent but <online database forget name> recommends 3' end aligned which can be right aligned for reverse strand genes)

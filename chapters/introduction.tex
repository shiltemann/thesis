...
Some concepts important to

\begin{enumerate}
\item accessibility of tools (open source, github etc)
\item reproducibility (version control)
\item maintainability (testing, continuous integration)
\item usability (galaxy, dependency management, docker)
\item results delivery (reporting/visualization)
\item data management (FAIR data, LIMS)
\item training
\end{enumerate}


\subsection{Usability}
Most bioinformatics tools are commandline tools, and most biologists are not trained in the use of such

even for bioinformaticians, running some of these tools can be a challenge as they are often not documented well. Just getting them installed can already be a challenge with all dependencies and OS differences

Galaxy provides a web interface to such tools, bringing analysis tools to the experts equiped to do the interpretations.
...
\item \textbf{Bob:} Charlie was our student, they left 6 months ago \\
\item \textbf{Alice:} How do I get this information now? \\
\item \textbf{Bob:} ... *crickets* ...
\end{itemize}
}

And this is not just a problem within a single institution; far too often, results described in papers are also impossible to reproduce given the information available in the manuscript.
...
\subsection{Finding the right tool}
many tools to do the same task. which is best? all claim superiority in manuscript. review articles may compare several, still an optimization problem (better at subtask A but worse at subtask B)
...
%\addcontentsline{toc}{section}{Use case 1: Prostate Cancer}

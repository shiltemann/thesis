...
\subsection{Large data, small reads}
Current sequencing techniques cannot simply read all the nucleotides of an entire chromosome at once; instead, the chromosomes are cut into tiny pieces of a few hundred bases, and each of those small fragments is sequenced. The full genome must then be reconstructed from those short sequences reads. Think of this as solving a jigsaw puzzle with billions of pieces. To complicate matters further, the nucleotides determined by the sequencer (\emph{base calls}) are not always correct. Furthermore, we don't sequence just one cell, but combine material from a large number of cells together. And of course the genome itself contains complications such as repetitive regions, that complicate the reconstruction further. In the case of human DNA, we have the reference genome as a guide, and can use this as a scaffold to map each of the reads to. For other organisms however, such a reference genome does not exist, and the short reads must be assembled \emph{de-novo} to reconstruct the genome. Think of this as solving a jigsaw puzzle with billions of pieces, but without the picture on the box to guide you.
...
Every cell in our body contains a copy of our genome in its nucleus, which has been dubbed the \emph{blueprint of life}. But these cells aren't the only source of genetic material in our bodies; each one of us harbour a vast amount of micro-organisms in various parts of our bodies. Estimates put the number of of microbial cells as equal to or slightly outnumbering human cells \cite{sender2016outnumbered}. These microorganisms can influence our metabolism, impact our health and immunity, and modify drug efficacy. If our genomes are the blueprint of life, then our microbiomes are an important overlay to this blueprint; or, when we consider the genome as the \emph{source code} of life, the microbiome is an essential third-party plugin. For this reason, the microbiome is often referred to as our \emph{second genome} \cite{grice2012microbiome}, and has emerged in recent years as an important field of biomedical study.

<TODO: some examples of microbiota impacting health/disease

\subsection{The Informatics}

Sequencing the human microbiome comes with its own unique set of challenges. Because the microbiome typically consists of a large number of different organisms, we cannot map to a single reference genome. it is like with cancer, multiple huge jigsaws

\paragraph{16S rRNA sequencing} is used in situations where we are primarily interested in the \emph{composition} of the microbiome; we only want to know which species of organisms are present, and are less concerned about the exact genome sequence or any mutations present. Consider for example when a patient has an infection and, depending on the species of bacteria present, we have different courses of treatment available (e.g. antibiotics).

For such cases, we do not sequence the whole genome of the microorganisms, but only a single gene of each genome. The 16S rRNA is ideally suited for this purpose because it is present in all bacteria, but contains enough variability between different species to be able to distinguish between them. Because we sequence a much smaller stretch of DNA, this approach is much cheaper than were we to sequence all DNA. Before the advent of sequencing, and as often still sufficient, bacteria from such samples are cultured in a petrie dish in order to identify the strains present, but with sequencing costs decreasing, 16S sequencing has become a viable alternative, with the added advatage that it is not restricted to aerobic organisms or other constraints imposed but the culture process.

...
\begin{comment}
Set the stage, lot's of data being generated, need to keep up on the analysis side as well
\end{comment}
...
Sequencing technologies are evolving at a staggering pace, and with them so must the software necessary to analyse and understand the data. As a consequence, new tools are developed continually and existing tools must be updated regularly to remain relevant. Typically there simply is no time for standards and consensus pipelines to emerge organically before new technologies make them obsolete. Therefore, there will be a multitude of tools available for any given task, and the challenge is to find the right tool for the job, and knowing when to use existing tools and when to develop new tools.

Lack of clear standards does not only apply to tools, but also to file formats, and this poses a major hurdle in data analysis. While some clear data format standards exist [fasta, VCF], there are many cases that lack such a standard, hindering the interoperability of different tools. When creating pipelines from existing tools, many file transformation steps are required as a \textit{glue} between steps.
...
All of these challenges have resulted in bioinformatics becoming a highly specialzed field, which in itself poses a new challenge by the fact that the domain knowledge (biology) and the bioinformatics know-how do not reside in a single individual, and the interpretation of the data cannot always be done by the person performing the data

has become such a specialized field, domain expertise and bioinformatics expertise needed for analysis usually not in 1 person, need to work together, empower domain scientists to perform their own analysis
